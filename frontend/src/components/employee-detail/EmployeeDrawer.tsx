...
import { getEmployeePlannedWorkedTimeline } from "@/api/excelUtilisation"
import type { EmployeeDetail } from "@/types/employee"
import type { EmployeeTimeline } from "@/types/project"
import type { PlannedWorkedEntry } from "@/api/excelUtilisation"
...
export function EmployeeDrawer() {
  ...
  const [plannedWorked, setPlannedWorked] = useState<PlannedWorkedEntry[]>([])
  const [selectedPlannedWorkedPeriod, setSelectedPlannedWorkedPeriod] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    setTimeline(null)
    setPlannedWorked([])
    setSelectedPlannedWorkedPeriod(null)

    const load = async () => {
      const [empResult, chainResult, timelineResult, pwResult] = await Promise.allSettled([
        getEmployee(selectedId, drawerPeriod ?? undefined, drawerDataSource),
        getReportingChain(selectedId),
        getEmployeeTimeline(selectedId),
        getEmployeePlannedWorkedTimeline(selectedId),
      ])
      ...
      if (chainResult.status === "fulfilled") setChain(chainResult.value.chain || [])
      if (timelineResult.status === "fulfilled") setTimeline(timelineResult.value)
      if (pwResult.status === "fulfilled") setPlannedWorked(pwResult.value.timeline || [])
  ...
  useEffect(() => {
    if (plannedWorked.length === 0) {
      setSelectedPlannedWorkedPeriod(null)
      return
    }
    const preferredPeriod = drawerPeriod && plannedWorked.some((entry) => entry.period === drawerPeriod)
      ? drawerPeriod
      : plannedWorked[plannedWorked.length - 1]?.period
    setSelectedPlannedWorkedPeriod((current) => (
      current && plannedWorked.some((entry) => entry.period === current)
        ? current
        : preferredPeriod ?? null
    ))
  }, [plannedWorked, drawerPeriod])

  if (!isOpen) return null

  const locColor = employee && !(employee as any).is_unmatched
    ? LOCATION_COLORS[employee.location_code] || "#6b7280"
    : "#6b7280"
  const selectedPlannedWorkedIndex = plannedWorked.findIndex((entry) => entry.period === selectedPlannedWorkedPeriod)
  const activePlannedWorked = selectedPlannedWorkedIndex >= 0
    ? plannedWorked[selectedPlannedWorkedIndex]
    : plannedWorked[plannedWorked.length - 1] ?? null
    ...
    <Sheet open={isOpen} onClose={closeDrawer}>
      {loading ? (
        ...
        <>
          ...
          <SheetContent>
            {/* Unmatched Excel employee — not registered in system */}
            {(employee as any).is_unmatched ? (
              <div className="space-y-4">
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  <p className="text-sm text-amber-700 font-medium">Not registered in system</p>
                  <p className="text-xs text-amber-600 mt-0.5">
                    This employee appears in the Excel report but has no profile in the HR system.
                  </p>
                </div>
                        ...
                        <div className="space-y-2">
                          {employee.managers.map((mgr) => (
                            ...
                            >
                              ...
                              <Badge variant="outline" className="text-[10px]">{mgr.relationship_type}</Badge>
                              <Badge variant="secondary" className="text-[10px]">{mgr.location_code}</Badge>
                            </button>
                          ))}
                        </div>
                      </div>
                            ...
                            >
                              <div className="flex items-center justify-between mb-1">
                                ...
                                <div className="flex gap-1">
                                  ...
                                  <Badge
                                    variant={proj.status === "ACTIVE" ? "default" : proj.status === "COMPLETED" ? "secondary" : "outline"}
                                    className="text-[10px]"
                                  >
                                    {proj.status}
                                  </Badge>
                              ...
                              <p className="text-xs text-muted-foreground mb-2">Role: {proj.role_in_project}</p>
                              <div className="flex items-center gap-2">
                                <Progress value={proj.progress_percent} className="flex-1" />
                                <span className="text-xs font-medium">{proj.progress_percent.toFixed(0)}%</span>
                              </div>
                                ...
                                >
                                  <div
                                    className={`w-8 h-8 rounded-sm transition-opacity hover:opacity-80 ${
                                      isBench ? "bg-red-200"
                                        : entry.status === "fully_billed" ? "bg-green-400"
                                        : entry.status === "partially_billed" ? "bg-amber-300"
                                        : "bg-blue-300"
                                    }`}
                                  />
                      ...
                      <div className="mb-6">
                        <div className="mb-3 flex items-center justify-between gap-2">
                          <h3 className="text-sm font-semibold flex items-center gap-1.5">
                            <BarChart3 className="h-4 w-4" /> Planned vs Actual (Days)
                          </h3>
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => {
                                if (selectedPlannedWorkedIndex > 0) {
                                  setSelectedPlannedWorkedPeriod(plannedWorked[selectedPlannedWorkedIndex - 1].period)
                                }
                              }}
                              disabled={selectedPlannedWorkedIndex <= 0}
                              className="inline-flex h-7 w-7 items-center justify-center rounded-md border text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:cursor-not-allowed disabled:opacity-40"
                            >
                              <ChevronLeft className="h-4 w-4" />
                            </button>
                            <span className="min-w-[96px] text-center text-xs font-medium">
                              {new Date(
                                Number(activePlannedWorked.period.split("-")[0]),
                                Number(activePlannedWorked.period.split("-")[1]) - 1
                              ).toLocaleDateString("en", { month: "long", year: "numeric" })}
                            </span>
                            <button
                              type="button"
                              onClick={() => {
                                if (selectedPlannedWorkedIndex >= 0 && selectedPlannedWorkedIndex < plannedWorked.length - 1) {
                                  setSelectedPlannedWorkedPeriod(plannedWorked[selectedPlannedWorkedIndex + 1].period)
                                }
                              }}
                              disabled={selectedPlannedWorkedIndex < 0 || selectedPlannedWorkedIndex >= plannedWorked.length - 1}
                              className="inline-flex h-7 w-7 items-center justify-center rounded-md border text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:cursor-not-allowed disabled:opacity-40"
                            >
                        ...
                        {(() => {
                          const entry = activePlannedWorked
                          const maxDays = Math.max(entry.planned_days, entry.worked_days, 1)
                          const over = entry.worked_days > entry.planned_days
                          return (
                            <div className="rounded-lg border p-2.5 space-y-1.5">
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-medium">Month Summary</span>
                                <span className={`text-[10px] font-medium tabular-nums ${over ? "text-red-600" : "text-green-700"}`}>
                                  {entry.worked_days}d / {entry.planned_days}d
                                </span>
                              </div>
                              <div className="space-y-0.5">
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] text-muted-foreground w-12 shrink-0">Planned</span>
                                  <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                                    <div className="h-full rounded-full bg-blue-400" style={{ width: `${(entry.planned_days / maxDays) * 100}%` }} />
                                  </div>
                                  <span className="text-[10px] tabular-nums w-8 text-right">{entry.planned_days}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] text-muted-foreground w-12 shrink-0">Actual</span>
                                  <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                                    <div className={`h-full rounded-full ${over ? "bg-red-400" : "bg-green-400"}`} style={{ width: `${(entry.worked_days / maxDays) * 100}%` }} />
                                  </div>
                                  <span className="text-[10px] tabular-nums w-8 text-right">{entry.worked_days}</span>
                                </div>
                              </div>
                                ...
                                <div className="pt-1 border-t space-y-0.5">
                                  {entry.projects.map((p, i) => (
                                    <div key={i} className="flex items-center justify-between text-[10px] text-muted-foreground">
                                      ...
                                      <span className="tabular-nums shrink-0">{p.worked_days}d / {p.planned_days}d</span>
                                    </div>
                                  ))}
                                </div>
                              )}
                      ...
                      {employee.direct_reports.map((report) => (
                        ...
                        >
                          <div
                            ...
                            style={{ backgroundColor: LOCATION_COLORS[report.location_code] || "#6b7280" }}
                          >
                            {report.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm truncate">{report.name}</p>
                            <p className="text-xs text-muted-foreground truncate">{report.designation}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                {!employee.is_own_branch && (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 mt-4">
                    <p className="text-sm text-amber-700">
                      {(employee as any).is_other_branch
                        ? "This employee is from another branch — Excel data is not available for cross-branch employees."
                        : "Limited access — this employee is in another branch. Contact info and project details are not available."}
                    </p>
                  </div>
                )}
              </>
